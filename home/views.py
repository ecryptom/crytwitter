...
def home(req):
    currency.objects.get(symbol='LINK').id
    return render(req, 'index.html', {'top_curs': currency.objects.filter(id__lt=currency.objects.get(symbol='USDC').id), 'dollor_rate':dollor.objects.get().rate})
